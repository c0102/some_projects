<script lang="ts">
    ...
    async function onClick(on_off: boolean) {
      ...
      await fetch("/api/toggle-led", {
        ...
        body: JSON.stringify({"cmd":"toggle_led", is_on: isOn }),
      });
    }
  </script>
  
  {#if isOn}
    <div on:click={() => onClick(false)} class="rounded-full border-4 border-black border-solid w-16 h-16 bg-red-600 cursor-pointer" />
  {:else}
