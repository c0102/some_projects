<script lang="ts">
  ...
  let password = "";
  const getAps = async () => {
    const res = await fetch("/api/get_aps");
    aps = await res.json();
    console.log(aps);
  };
  onMount(getAps);
  const joinNetwork = async () => {
    try {
      const result = await fetch("/api/get_aps", {
        method: "POST",
...
{:else}
  <select bind:value={ssid} class="select select-bordered w-full max-w-xs">
    <option disabled selected>Select an AP to connect to. (NOTE: not working at the moment)</option>
    {#each aps as ap}
