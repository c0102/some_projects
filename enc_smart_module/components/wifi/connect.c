...
void event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    {
        //TODO(uran): Handle the dosconnect better! Especially in the case that WiFi disconected after connecting
        ...
        ESP_LOGE(TAG, "disconnected: %s ,No. connecting tries %d, remaining %d tries", err, try_count, 15-try_count);
        try_count++;
        if(try_count < 15)
            esp_wifi_connect();
        //xEventGroupSetBits(wifi_events, DISCONNECTED);
...
esp_err_t wifi_connect_sta(const char *ssid, const char *pass, int timeout, int tries)
{
    ...
    while (result != CONNECTED_GOT_IP && --tries > 0)
    {
        ESP_LOGI(TAG, "Trying to connect again,  %d tries left", tries);
        esp_wifi_connect();
        result = xEventGroupWaitBits(wifi_events, CONNECTED_GOT_IP | DISCONNECTED, pdTRUE, pdFALSE, pdMS_TO_TICKS(timeout));
        
    }

    // return ESP_OK if CONNECTED_GOT_IP, return ESP_FAIL otherwise
    return result == CONNECTED_GOT_IP ? ESP_OK : ESP_FAIL;
    // if(result == CONNECTED_GOT_IP ) return ESP_OK;
    // return ESP_FAIL;
}
...
void wifi_connect_ap_sta(const char *ssid, const char *pass )
{
    ...
    ESP_LOGI(TAG, "Wifi ap starting with ssid: %s and pass: %s", ssid, pass);
