...
# Thread for deviceConfiguration
def deviceConfigThread(client):
    while not stop_flag.is_set():
        if configDeviceBuffer:
            handleDeviceConfig(client, configDeviceBuffer.pop())


def handleDeviceConfig(client, incomming_message):
...
# According to the type of the operation read from the JSON the data is saved in the tables where the data belongs.
    if jsonFormat:
        if(data["operation"] == "addDevice"):
            deviceConfig.startScan(client=client, data=incomming_message)
...
def subscribeToDevices(client):
    ...
    client.subscribe('hub01/devconf')
...
def on_message(client, userdata, msg):
    ...
    elif(msg.topic == "hub01/devconf"):
        configDeviceBuffer.append(incomming_message)
        print("heree!", incomming_message)
...
configThread = threading.Thread(target=deviceConfigThread, args=(client,))
configThread.start()
